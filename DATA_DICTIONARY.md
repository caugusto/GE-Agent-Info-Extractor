...
| `agent_published_version` | `STRING` | Version tag or release ID. | `v1.0`, `1.0.0`, `v1.0-a2a`, `Draft`, `N/A` | Extracted from version fields or defaults based on deployment state. |
| `agent_published_date` | `TIMESTAMP` | Timestamp when agent was published. | UTC Timestamp or `NULL` | Extracted from publish timestamp if published. |
| `agent_status` | `STRING` | Operational and sharing status.<br><br>**Extraction Sources:**<br>• **Discovery Engine**: `state` (`ENABLED` -> `Enabled`, `PRIVATE` -> `Private`, `DRAFT` -> `Draft`)<br>• **Reasoning Engine**: `Enabled`<br>• **Cloud Run**: Service readiness (`Ready` condition -> `Active (A2A Endpoint)`) | • `Enabled`<br>• `Private`<br>• `Draft`<br>• `Active (A2A Endpoint)` | Derived directly from Discovery Engine agent state or Cloud Run service readiness status. |
...
> - **Gemini Enterprise Registered Agents** (`Agent Designer`, registered `Agent Runtime`, `Dialogflow`, `Workflow Agent`, `Google Built-in Agent`, `A2A Agent`, `App Engine`): Extracted from Discovery Engine IAM policies (`getIamPolicy`).
